...
function AddJobs() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token"); // Adjust based on your storage logic
      console.log(token + " is the token");

      const response = await axios.post(
        "http://localhost:8080/add-jobs",
        jobData,
        {
          headers: {
      ...
      console.log(error.response ? error.response.data : error.message); // Log the specific error
    }
  };
